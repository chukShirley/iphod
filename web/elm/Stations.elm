port module Stations exposing (..) -- where
...
init: (Model, Cmd Msg)
init = (initModel, requestStation "1")
...
update msg model =
  case msg of
    ...
    UpdateStation newModel -> (newModel, Cmd.none)

    Next -> 
      let
        x = Result.withDefault 0 (String.toInt model.id) + 1
        next = if x > 14 then 1 else x
        newCmd = requestStation (toString next)
      in
        (model, newCmd)
...
navigationButtons model =
  let
    thisStation n = if n == model.id
      then style [("background-color", "lightgrey"), ("color", "darkblue")]
      else style []
  in
    p []
